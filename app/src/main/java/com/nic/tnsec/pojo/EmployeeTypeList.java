package com.nic.tnsec.pojo;

public class EmployeeTypeList {

    String empcode_type;
    String empcode_description;

    public String getEmpcode_type() {
        return empcode_type;
    }

    public EmployeeTypeList setEmpcode_type(String empcode_type) {
        this.empcode_type = empcode_type;
        return this;
    }

    public String getEmpcode_description() {
        return empcode_description;
    }

    public EmployeeTypeList setEmpcode_description(String empcode_description) {
        this.empcode_description = empcode_description;
        return this;
    }
}
